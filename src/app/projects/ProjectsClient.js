...
function ProjectNexusIcon() {
  return (
    <svg className="w-full h-full" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="50" cy="50" r="30" stroke="#E11D48" strokeWidth="2" strokeDasharray="4 4" className="animate-spin-slow" />
      <path d="M 50 20 L 50 80 M 20 50 L 80 50" stroke="#E11D48" strokeWidth="1.5" opacity="0.6" />
      <polygon points="50,35 65,50 50,65 35,50" fill="#E11D48" opacity="0.2" />
      <circle cx="50" cy="50" r="6" fill="#E11D48" className="animate-pulse" />
    </svg>
  );
}

function QuantizedCosmosIcon() {
  return (
    <svg className="w-full h-full" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="25" y="25" width="50" height="50" rx="4" stroke="#06B6D4" strokeWidth="2" strokeDasharray="6 2" />
      <line x1="25" y1="25" x2="75" y2="75" stroke="#06B6D4" strokeWidth="1.5" opacity="0.5" />
      <line x1="75" y1="25" x2="25" y2="75" stroke="#06B6D4" strokeWidth="1.5" opacity="0.5" />
      <circle cx="50" cy="50" r="10" stroke="#06B6D4" strokeWidth="1.5" fill="#09090e" />
      <circle cx="50" cy="50" r="4" fill="#06B6D4" className="animate-ping" style={{ animationDuration: "2s" }} />
    </svg>
  );
}

function CineSuggestIcon() {
  return (
    <svg className="w-full h-full" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="20" y="25" width="60" height="50" rx="4" stroke="#F59E0B" strokeWidth="2.5" />
      <path d="M 35 25 L 35 75 M 65 25 L 65 75" stroke="#F59E0B" strokeWidth="1.5" strokeDasharray="3 3" />
      <polygon points="45,42 60,50 45,58" fill="#F59E0B" className="animate-pulse" />
      <circle cx="50" cy="50" r="28" stroke="#F59E0B" strokeWidth="1.5" opacity="0.3" className="animate-spin-slow" />
    </svg>
  );
}
...
const projectsData = [
  ...
  },
  {
    ...
    description: "An autonomous multi-agent developer sandbox that plans, executes, and edits codebases from high-level specifications.",
    tags: ["AI Agents", "Automated Coding", "Next.js", "AGI"],
    status: "CONCEPT",
    color: "#E11D48",
    glowColor: "rgba(225, 29, 72, 0.15)",
    badgeBg: "bg-rose-500/10 text-rose-400 border-rose-500/20",
    icon: <ProjectNexusIcon />
  },
  {
    slug: "quantized-cosmos",
    name: "Quantized Cosmos",
    timeline: "2028 Vision",
    description: "A high-performance WebGL volume renderer visualizing quantum-inspired spatial databases and neural fields in 3D.",
    tags: ["WebGL", "Shaders", "Spatial Data", "Three.js"],
    status: "CONCEPT",
    color: "#06B6D4",
    glowColor: "rgba(6, 182, 212, 0.15)",
    badgeBg: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20",
    icon: <QuantizedCosmosIcon />
  },
  {
    slug: "cinesuggest-ai",
    name: "CineSuggest AI",
    timeline: "2026 Winter Vision",
    description: "An AI movie recommendation engine analyzing viewer micro-behaviors, sentiment reviews, and social graph models.",
    tags: ["Recommender Engine", "NLP", "PyTorch", "Collaborative Filtering"],
    status: "PLANNING",
    color: "#F59E0B",
    glowColor: "rgba(245, 158, 11, 0.15)",
    badgeBg: "bg-amber-500/10 text-amber-400 border-amber-500/20",
    icon: <CineSuggestIcon />
  }
];

function ProjectCard({ project, isFuture = false, isHovered, isAnyHovered, onHoverStart, onHoverEnd }) {
  const isBlurred = isAnyHovered && !isHovered;
  
  return (
    <Link
      href={`/projects/${project.slug}`}
      className={`group relative w-full rounded-2xl p-5 md:p-6 flex flex-col md:flex-row items-center gap-6 transition-all duration-300 ${
        isFuture 
          ? "bg-slate-950/25 border border-dashed border-slate-900/60" 
          : "bg-slate-950/45 border border-slate-900/60"
      } ${
        isBlurred ? "opacity-25 scale-[0.98] blur-[0.5px]" : "opacity-100 scale-100"
      } ${
        isHovered ? "-translate-y-2 scale-[1.005]" : ""
      }`}
      style={{
        borderColor: isHovered ? `${project.color}45` : undefined,
        backgroundColor: isHovered ? (isFuture ? 'rgba(5, 5, 8, 0.45)' : 'rgba(5, 5, 8, 0.75)') : undefined,
        boxShadow: isHovered ? `0 20px 50px rgba(0, 0, 0, 0.75), inset 0 0 16px ${project.color}22` : '0 4px 20px rgba(0, 0, 0, 0.2)'
      }}
    ...
    >
      ...
      <div 
        className="absolute left-0 top-3 bottom-3 w-[3px] rounded-r-md transition-all duration-300 origin-center scale-y-0 group-hover:scale-y-100 opacity-0 group-hover:opacity-100"
      ...
      <div 
        className="w-24 h-24 shrink-0 rounded-2xl bg-black/55 border flex items-center justify-center p-3 relative overflow-hidden transition-all duration-300 group-hover:scale-[1.06]"
        style={{
          borderColor: isHovered ? `${project.color}50` : '#111827',
      ...
      <div className="flex flex-col gap-2 text-left flex-1 w-full">
        <div className="flex flex-wrap items-center gap-3">
          <h2 
            className="font-orbitron text-lg md:text-xl font-bold uppercase tracking-wider transition-colors duration-305"
            style={{ color: isHovered ? '#FFFFFF' : (isFuture ? '#CBD5E1' : '#FFFFFF') }}
          >
            {project.name}
          </h2>
          <span 
            className={`text-[9px] font-space-mono px-2 py-0.5 rounded border uppercase tracking-widest font-black ${
              isFuture
                ? "text-[#10B981] bg-emerald-950/20 border-emerald-900/30"
                : "text-[#A855F7] bg-purple-950/20 border-purple-900/30"
            }`}
          >
            {project.timeline}
          </span>
        </div>
        <p className={`font-sans text-sm leading-relaxed max-w-2xl transition-colors duration-305 ${isHovered ? 'text-slate-200' : 'text-slate-400'}`}>
          {project.description}
        </p>
      ...
      <div className="flex items-center justify-between md:justify-end gap-5 shrink-0 w-full md:w-auto mt-4 md:mt-0 border-t border-slate-900/60 md:border-t-0 pt-4 md:pt-0">
        ...
        <div 
          className="w-10 h-10 rounded-full bg-slate-950/60 border flex items-center justify-center transition-all duration-300 group-hover:translate-x-1 shadow-sm"
          style={{
            borderColor: isHovered ? `${project.color}50` : '#111827',
...
export default function ProjectsLabPage() {
  const activeProjects = projectsData.filter(p => p.status !== "PLANNED" && p.status !== "PLANNING" && p.status !== "CONCEPT");
  const futureProjects = projectsData.filter(p => p.status === "PLANNED" || p.status === "PLANNING" || p.status === "CONCEPT");
  ...
  return (
    <main className="relative w-screen h-screen overflow-y-auto overflow-x-hidden bg-[#050508] text-white select-none pb-20 scrollbar-thin">
      ...
      <div className="max-w-4xl mx-auto w-full px-6 py-24 flex flex-col gap-12 mt-10">
        ...
        <div className="flex flex-col gap-6 w-full mt-4">
          <div className="flex items-center gap-3 w-full border-b border-slate-900/60 pb-3">
            <span className="w-1.5 h-1.5 rounded-full bg-purple-500 shadow-[0_0_8px_#A855F7] animate-pulse" />
            <h2 className="font-orbitron text-[10px] tracking-[0.25em] font-black uppercase text-slate-400">
              ACTIVE EXPERIMENTS
            </h2>
          </div>
